...
} from '@interview-app/shared/src/prompts/index.js';
...
export class AiService {
  ...
  async evaluateAnswer(question: any, userAnswer: string) {
    const userPrompt = QUIZ_EVALUATION_USER_PROMPT_TEMPLATE
      .replace('{{question}}', question.question)
      .replace('{{expected_key_points}}', (question.expectedKeyPoints || []).join(', '))
  ...
  async extractKnowledgeGaps(
    ...
    const userPrompt = KNOWLEDGE_GAP_USER_PROMPT_TEMPLATE
      .replace('{{note_topic}}', topic)
